...
const MesaDePartesLogin = () => {
  const navigate = useNavigate();
  const { loginMesaDePartes } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await loginMesaDePartes({
        CIP: formData.CIP,
        ...
        <div className="bg-white dark:bg-dark-surface transition-colors rounded-2xl p-8 w-96">
          ...
          <div className="text-center mb-8">
            <div className="mb-5">
              ...
              <h2 className="text-lg text-gray-600 dark:text-dark-text-secondary">Acceso de usuario de Mesa de Partes</h2>
            </div>
          </div>
...
export default MesaDePartesLogin;
