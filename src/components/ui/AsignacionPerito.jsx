// Este componente ahora es un "dumb component" que solo renderiza el modal.
// El control del estado (abrir/cerrar, cargar datos) se maneja en el componente padre (CrearOficio).
const AsignacionPerito = ({ isModalOpen, peritos, onClose, onPeritoSelect, loading }) => {
  if (!isModalOpen) {
    return null;
  }
    ...
    <div className="fixed inset-0 bg-transparent backdrop-blur-md flex justify-center items-center z-50">
      <div className="bg-white dark:bg-dark-bg-secondary rounded-xl shadow-2xl p-6 w-full max-w-2xl">
        ...
        <div className="overflow-y-auto max-h-[60vh]">
          {loading ? (
            <div className="text-center p-6">Cargando peritos...</div>
          ) : (
                ...
                {peritos.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan="4" className="text-center p-6 text-gray-500 dark:text-gray-400">
                      No hay peritos disponibles para esta combinación de exámenes.
                    </td>
                  </tr>
                )}
              </tbody>
