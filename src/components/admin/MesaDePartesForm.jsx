...
const MesaDePartesForm = () => {
  ...
  const {
    ...
    validate: (vals) => {
      ...
      if (!vals.nombre_usuario || !vals.nombre_usuario.trim())
        errs.nombre_usuario = "Nombre de usuario es requerido";
      if (!vals.nombre_completo || !vals.nombre_completo.trim())
        errs.nombre_completo = "Nombre completo es requerido";
      if (!isEditing && !vals.password_hash)
        errs.password_hash = "Contraseña es requerida";
      if (vals.password_hash && vals.password_hash !== vals.confirmar_password)
        errs.confirmar_password = "Las contraseñas no coinciden";
      return errs;
    },
    onSubmit: async (formData) => {
      try {
        ...
        if (isEditing) {
          ...
          const res = await MesaDePartes.updateUserMesaDePartes(
            cip,
            updateData
          );
          if (res?.error)
            throw new Error(
              res.error || res.message || "Error actualizando usuario"
            );
          ...
          if (res?.error)
            throw new Error(
              res.error || res.message || "Error creando usuario"
            );
